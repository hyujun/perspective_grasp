...
> `models/` vs `runtime_outputs/`: `<repo>/models/` 는 **input asset** 전용 (가중치, 메시, 데이터셋, YOLO가 자동 다운로드한 `yolov8n.pt` 포함). 노드가 런타임에 만들어내는 산출물 (캘리브레이션 결과, BundleSDF per-track 덤프, 디버그 녹화 등) 은 `<repo>/runtime_outputs/<subdir>/` 로 떨어진다. 두 디렉토리 모두 git-ignore. 런치 파일에서는 `perception_launch_utils.workspace_models_dir()` / `workspace_runtime_outputs_dir(subdir)` 로 해결하고, 필요하면 `$PERSPECTIVE_GRASP_{REPO_ROOT,MODELS_DIR,RUNTIME_OUTPUTS_DIR}` 로 외부 디스크에 매핑하거나 deploy PC에서 repo 폴더명이 `perspective_grasp/` 가 아닐 때 walk-up fallback을 우회한다 — 자세한 건 [installation.md § Workspace layout](./installation.md#workspace-layout).
