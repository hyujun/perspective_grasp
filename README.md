...
3. **Host profiles.** `host_profile:=<auto|dev_8gb|prod_16gb|cpu_only>` (env: `PERSPECTIVE_HOST_PROFILE`) selects parameter overrides keyed by node name from [`packages/infrastructure/perception_launch_utils/host_profiles/`](packages/infrastructure/perception_launch_utils/host_profiles/) (shipped with `perception_launch_utils` so Phase 4 Docker images can resolve them without needing `perception_bringup`). Profiles only swap **parameters** (model size, batch, mock vs real backend) — never code paths. `auto` picks via total VRAM.
