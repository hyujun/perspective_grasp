  ...
  pipeline_mode_("UNKNOWN")
{
  ...
  declare_parameter<std::string>("masks_topic_suffix", kDefaultMasksSuffix);
  declare_parameter<int>("active_camera_index", 0);
  declare_parameter<bool>("show_masks", true);
  ...
  const std::string masks_suffix = get_parameter("masks_topic_suffix").as_string();
  ...
  show_masks_ = get_parameter("show_masks").as_bool();
  mask_alpha_ = std::clamp(get_parameter("mask_alpha").as_double(), 0.0, 1.0);
  ...
  for (std::size_t i = 0; i < camera_namespaces_.size(); ++i) {
    ...
    RCLCPP_INFO(
      ...
      i, ns.c_str(), image_topic.c_str(), det_topic.c_str(), masks_topic.c_str());
  ...
  std::size_t cam_index, const sensor_msgs::msg::Image::ConstSharedPtr & msg)
{
  ...
  try {
    ...
    if (show_masks_ && cam_index < latest_masks_.size() && latest_masks_[cam_index]) {
      const std::size_t skipped = detail::draw_segmentations(
        cv_ptr->image, *latest_masks_[cam_index], mask_alpha_);
      if (skipped > 0) {
        RCLCPP_WARN_THROTTLE(
          get_logger(), *get_clock(), 5000,
          "draw_segmentations skipped %zu mask(s) (size mismatch or empty data)",
          skipped);
      }
    }
...
void VisualizerNode::segmentations_callback(
  ...
  const std::vector<rclcpp::Parameter> & params)
{
  ...
  for (const auto & p : params) {
    ...
    } else if (p.get_name() == "show_masks") {
      show_masks_ = p.as_bool();
      RCLCPP_INFO(get_logger(), "show_masks → %s",
                  show_masks_ ? "true" : "false");
    } else if (p.get_name() == "mask_alpha") {
      const double v = p.as_double();
      if (v < 0.0 || v > 1.0) {
        result.successful = false;
        result.reason = "mask_alpha must be in [0, 1]";
        return result;
      }
      mask_alpha_ = v;
      RCLCPP_INFO(get_logger(), "mask_alpha → %.3f", mask_alpha_);
    }
  }
  return result;
}
