...
"""Shared yaml loader for perception_bringup camera configs.

Installed to ``share/perception_bringup/launch/`` so that other launch files
can import it via ``importlib`` without relying on ``sys.path`` magic.
...
Example::
    ...
    def _load():
        ...
        spec = importlib.util.spec_from_file_location('cc_loader', path)
        mod = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(mod)
        return mod

    loader = _load()
    cameras = loader.load_cameras('/path/to/camera_config.yaml')
    namespaces = [c.namespace for c in cameras]
"""
