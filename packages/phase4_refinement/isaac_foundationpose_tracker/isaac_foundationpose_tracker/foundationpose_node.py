"""FoundationPose 6D-pose tracker LifecycleNode.

Pairs YOLO detections with aligned RGB-D frames (by timestamp), runs the
configured pose backend per detection using its mesh/class registry,
and publishes results as ``PoseWithMetaArray`` on
``/foundationpose/raw_poses`` (or ``/{cam_ns}/foundationpose/raw_poses``
under multi-camera fan-out).

Concrete inference lives in ``backends/`` so the node stays framework-
agnostic — ``foundationpose`` for the Docker service, ``mock`` for
CPU-only smoke tests. The backend contract is the same one SAM2 uses,
just adapted for pose output (see ``backends/base_backend.py``).
"""
...
from isaac_foundationpose_tracker.backends.base_backend import (
    BasePoseBackend,
    PosePrompt,
    PoseResult,
)
...
BACKEND_REGISTRY: dict[str, str] = {
    'foundationpose':
        'isaac_foundationpose_tracker.backends.foundationpose_backend'
        ':FoundationPoseBackend',
    'mock':
        'isaac_foundationpose_tracker.backends.mock_backend:MockPoseBackend',
}


def _load_backend(key: str, node) -> BasePoseBackend:
    ...
    if not issubclass(cls, BasePoseBackend):
        raise TypeError(f'{cls.__name__} is not a BasePoseBackend')
    return cls(node)


class FoundationPoseNode(LifecycleNode):
    """LifecycleNode wrapping an RGB-D 6D pose backend."""
    ...
    def __init__(self) -> None:
        ...
        self._poses_topic: str = (
            self.declare_parameter('poses_topic', '/foundationpose/raw_poses')
            .get_parameter_value().string_value
        )
        ...
        self._backend: BasePoseBackend | None = None
        self._K: np.ndarray | None = None
        self._frame_lock = threading.Lock()
        # (stamp_ns, rgb, depth_m, frame_id)
        self._frame_queue: deque[tuple[int, np.ndarray, np.ndarray, str]] = deque()
        # RGB and depth arrive on different topics; stage them until a matching
        # depth frame joins its RGB pair, then enqueue together.
        ...
        self.get_logger().info(
            f"FoundationPoseNode created (backend='{self._backend_type}')"
        )
    ...
    def on_configure(self, state: LifecycleState) -> TransitionCallbackReturn:
        ...
        self.get_logger().info(
            ...
            f'info={self._camera_info_topic} dets={self._detections_topic}  '
    ...
    def _detections_callback(self, msg: DetectionArray) -> None:
        ...
        try:
            results = self._backend.estimate(rgb, depth_m, self._K, prompts)
        except Exception as e:  # noqa: BLE001
...
def _depth_to_metres(bridge: CvBridge, msg: Image) -> np.ndarray | None:
    """Convert a depth Image to float32 metres.

    Supports the two common RGB-D encodings: 16UC1 (millimetres) and
    32FC1 (metres).
    """
...
def _to_pose_with_meta(res: PoseResult, header) -> PoseWithMeta:
    pw = PoseWithMeta()
    pw.object_id = int(res.object_id)
    pw.class_name = res.class_name
    pw.source = 'foundationpose'
...
def _rot_to_quat(R: np.ndarray) -> tuple[float, float, float, float]:
    ...
    Uses Shepperd's method (branch on the largest diagonal) — numerically
    stable across the full SO(3) range without pulling in scipy.
    """
    m00, m01, m02 = R[0]
    m10, m11, m12 = R[1]
    m20, m21, m22 = R[2]
